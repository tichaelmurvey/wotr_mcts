...
def define_options_declare(fellowship: Fellowship):
    dest_options = tuple(
        MoveOption(MT.DECLARE_FELLOWSHIP, region)
        for region in get_n_regions_away(
            fellowship.region.idx, fellowship.track_position, fellowship.regions
        )
    )
...
def define_options_guide(fellowship: Fellowship):
    highest_level = max(
        CHARACTER_STATS[companion].level for companion in fellowship.companions
    )
    options = tuple(
        MoveOption(MT.CHANGE_GUIDE, companion)
        for companion in fellowship.companions
        if CHARACTER_STATS[companion].level == highest_level
    )
...
def define_options_hunt_allocation(game_env: WotrGame):
    ...
    options = tuple(
        MoveOption(MT.HUNT_ALLOCATION, num) for num in range(min_eyes, max_eyes)
    )
    return MoveOptionSet(options)
